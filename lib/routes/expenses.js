...
async function get (req, res) {
    ...
    try {
        ...
        // We just want to send an empty array when there is no data found, But postgres returns an error.
...
function groupExpensesByDay (expenses) {
    ...
    _.times(moment(expenses[0]["purchase_date"]).daysInMonth(), day => grouped[day + 1] = 0);
    expenses.forEach(expense => {
        grouped[moment(expense["purchase_date"]).date()] += expense.price;
    });
